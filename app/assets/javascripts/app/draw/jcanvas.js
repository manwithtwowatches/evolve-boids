...
// Get surrounding pixels in flat array
fn.getPixels = function(args){
	var params = extend({}, prefs, args),
	    ctx, elem, imgData;

	elem = this[0];
	ctx = elem.getContext('2d');
	imgData = ctx.getImageData(params.x, params.y, params.width, params.height);
	return imgData.data;
}

// Set pixels to specific block
fn.setPixelBlock = function(args) {
	var ctx, elem, e, i,
		params = extend({}, prefs, args),
		imgData, data, len, px;
